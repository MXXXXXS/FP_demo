...
现在可以复用`plusOneAndSquar`这个操作流程了, 相比之前手动嵌套好了很多

那么是否可以用`compose`来处理上面的`filter, sortBy`呢？
...
为了解决上面的`compose`无法组合具有多参的函数问题，这里引入了 Currying 与 Partial application 两种技巧
